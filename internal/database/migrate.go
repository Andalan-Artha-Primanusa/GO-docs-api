...
func Migrate(db *sql.DB) error {
	...
	return migrateExisting(db)
}

func migrateExisting(db *sql.DB) error {
	hasSiteLocationID, err := columnExists(db, "users", "site_location_id")
	if err != nil {
		return err
	}
	if !hasSiteLocationID {
		if _, err := db.Exec(`ALTER TABLE users ADD COLUMN site_location_id BIGINT NULL AFTER department_id`); err != nil {
			return err
		}
	}
	if ok, err := columnExists(db, "request_types", "sla_days"); err != nil {
		return err
	} else if !ok {
		if _, err := db.Exec(`ALTER TABLE request_types ADD COLUMN sla_days INT NULL AFTER is_active`); err != nil {
			return err
		}
	}
	if ok, err := columnExists(db, "requests", "due_at"); err != nil {
		return err
	} else if !ok {
		if _, err := db.Exec(`ALTER TABLE requests ADD COLUMN due_at TIMESTAMP NULL AFTER current_stage`); err != nil {
			return err
		}
	}
...
func columnExists(db *sql.DB, tableName, columnName string) (bool, error) {
	...
	err := db.QueryRow(`
		...
		WHERE table_schema = DATABASE()
		  AND table_name = ?
		  AND column_name = ?
		LIMIT 1`, tableName, columnName).Scan(&exists)
